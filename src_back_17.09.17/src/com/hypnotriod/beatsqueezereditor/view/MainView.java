...
public class MainView extends BaseView
{
        ...
        public void handle(DragEvent event) {
            ...
            if (db.hasFiles()) {
                ...
                for (File file:db.getFiles()) {
                    ...
                    fileExtention = "." + new FileName(filePath, '/', '.').extension();
    ...
    protected void handleVCNotification(String name, Object data) 
    {
        switch(name)
        {
            ...
            case MainSceneController.ON_SAMPLE_PLAY:
                performPlaySample((SampleVO)data);
                break;
    ...
    private synchronized void performPlaySample(SampleVO sampleVO)
    {
        SustainLoopVO loop;
    ...
    private void performSamplesLoadOnDrag(SampleVO sampleVO, File file)
    {
        ...
        thread = new Thread(new Runnable() {
            @Override
            public void run() {
                getFacade().loadSamplesController.manageAdditionalSample(file, sampleVO.fileRealName + sampleVO.selectedSampleExt);
                Platform.runLater(new Runnable() {
                    @Override
                    public void run() {
                        _mainSceneController.hideLoading();
    ...
    private void performSamplesLoad(List<File> withSamples)
    {
        ...
        if(files != null) {
            ...
            thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    getFacade().loadSamplesController.loadSamples(files, pitchStep);
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            _mainSceneController.hideLoading();
