...
public class LoadSamplesController extends BaseController
{
        ...
        for (String key : getMainModel().sampleVOs.keySet()) 
        {
            ...
            if(sampleVO.fileRealName.equals(fileNameNoExt))
            {
                try {
                    ...
                    parseLoopPoints(sampleVO, sampleRate, extStr, sampleVO.waveHeader.channels);
    ...
    private void parseLoopPoints(SampleVO sampleVO, int sampleRate, String extStr, int channels)
    {
        ...
        if(loop != null)
        {
            ...
            if(extStr.equals(CConfig.EXT_P)) sampleVO.samplesDataP = samplesData;
            else if(extStr.equals(CConfig.EXT_F)) sampleVO.samplesDataF = samplesData;
            else sampleVO.samplesData = samplesData;
        }
    }
    
    private byte[] getSamplesDataByExtention(String extStr, SampleVO sampleVO) {
        if(extStr.equals(CConfig.EXT_P)) return sampleVO.samplesDataP;
        else if(extStr.equals(CConfig.EXT_F)) return sampleVO.samplesDataF;
        else return sampleVO.samplesData;
    }
